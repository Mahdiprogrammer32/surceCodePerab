...
// تابع نمایش پیام‌های SweetAlert
...
// بررسی وجود پارامتر user_id
if (!isset($_GET['user_id']) || empty($_GET['user_id'])) {
    showAlert('خطا', 'شناسه کاربر مشخص نشده است.', 'error');
    exit;
}
...
if ($id === false) {
    showAlert('خطا', 'شناسه کاربر معتبر نیست.', 'error');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($totalPercentage > 60) {
        ...
        // محاسبه درصد پیشرفت
        $initialProgress = 0;
        if (!empty($employer)) $initialProgress += 5;
        if (!empty($contractor)) $initialProgress += 5;
        ...
        $updateStmt->bind_param(
            "iiiiiidddddssii",
            $employer, $contractor, $contractor2, $contractor3, $contractor4, $contractor5,
            $c1p, $c2p, $c3p, $c4p, $c5p, $phase_desc, $priority, $initialProgress, $projectID
        );
            ...
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = 'index.php?status=0&priority=on&abc=0&progres=on&start_date=&end_date=';
                    }
                });
            });
            </script>";
...
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
        ...
        <!-- نمایش مجموع درصدها -->
        <div class="alert alert-percentage alert-info" id="totalPercentageDisplay">
            مجموع درصدها: <span id="totalPercentage">0</span>%
        </div>
        <div class="alert alert-danger mt-2" id="percentageWarning" style="display: none;"></div>
...
<script>
    $(document).ready(function() {
        ...
        let subcontractorCount = <?= $subcontractorCount ?>;
        ...
        $('body').on('input', '.percentage-input', function() {
